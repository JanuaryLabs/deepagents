...
The `mssql` driver is a peer dependency. Install it alongside `@deepagents/text2sql`:

```bash
npm install @deepagents/text2sql mssql
```

## Basic Setup

Reuse the shared `model`, `filesystem`, and `context` setup from
[Getting Started](/docs/text2sql/getting-started) in each example.
...
| `columnValues()` | Detects enum-like and low-cardinality columns using `SELECT DISTINCT TOP` |
