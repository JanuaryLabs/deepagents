...
Complete pipeline with personas, a manual fragment bundle, evolution, and validation:

```typescript
import { hint, term } from '@deepagents/context';
import {
  SchemaSynthesizer,
  PersonaGenerator,
  BreadthEvolver,
...
const teachings = [
  ...
  hint("Exclude sandbox accounts where account_type = 'test'"),
];
...
| Manual teachings array | Want domain-aware questions | Business rules for synthesis |
