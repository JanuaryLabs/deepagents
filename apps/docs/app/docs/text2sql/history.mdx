...
Text2SQL writes conversation messages and chat metadata through the underlying
`ContextEngine`. That gives later `chat()` calls access to prior turns without a
separate history API on `Text2Sql` itself.
