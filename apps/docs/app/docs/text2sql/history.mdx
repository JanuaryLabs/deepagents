...
comes from the `ContextStore` backing the `ContextEngine` you pass to
`Text2Sql`. `chat()` writes turns to that store; `toSql()` does not persist
...
that usually means reusing the same `Text2Sql` instance or creating a new one
with a new `ContextEngine` built from the same thread identity. Pass only the
new incoming message for that turn; earlier turns are loaded from the store
automatically:

```typescript
await engine.continue(user('Show me sales data'));
for await (const _ of await text2sql.chat({ generateTitle: true })) {
  // drain or stream chunks to your UI
}

await engine.continue(user('Filter that to last quarter'));
for await (const _ of await text2sql.chat()) {
  // drain or stream chunks to your UI
}
```
...
- the incoming user or assistant message for that turn (queued by `engine.continue(...)`)
