...
Each database adapter (PostgreSQL, SQLite, SQL Server, MySQL/MariaDB, BigQuery, and ClickHouse) exports grounding factory functions. Import them and pass to your adapter:

Reuse the shared `model` setup from
[Getting Started](/docs/text2sql/getting-started) in the examples below.

<Tabs groupId="database" items={['PostgreSQL', 'SQLite', 'SQL Server', 'MySQL/MariaDB', 'BigQuery', 'ClickHouse']}>
