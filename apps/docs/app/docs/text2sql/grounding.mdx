...
Each database adapter (PostgreSQL, SQLite, DuckDB, SQL Server, MySQL/MariaDB, BigQuery, and ClickHouse) exports grounding factory functions. Import them and pass to your adapter:

Reuse the shared `model` setup from
[Getting Started](/docs/text2sql/getting-started) in the examples below.

<Tabs groupId="database" items={['PostgreSQL', 'SQLite', 'DuckDB', 'SQL Server', 'MySQL/MariaDB', 'BigQuery', 'ClickHouse']}>
...
Policy scope checks use only entity-producing groundings (`tables()` and
`views()`) to resolve the allowed table and view names. Metadata groundings such
as `indexes()`, `constraints()`, `rowCount()`, `columnStats()`, and
`columnValues()` annotate schema context during indexing, but they do not expand
the allowlist used by `validate()` or `execute()`.
...
- [DuckDB Adapter](/docs/text2sql/duckdb) - DuckDB-specific configuration
- [SQL Server Adapter](/docs/text2sql/sqlserver) - SQL Server-specific configuration
- [MySQL/MariaDB Adapter](/docs/text2sql/mysql) - MySQL and MariaDB-specific configuration
- [BigQuery Adapter](/docs/text2sql/bigquery) - BigQuery-specific configuration
- [ClickHouse Adapter](/docs/text2sql/clickhouse) - ClickHouse-specific configuration
