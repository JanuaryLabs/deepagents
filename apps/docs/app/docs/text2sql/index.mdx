...
If you already own the bash sandbox and want to wire the `sql` command
manually, see [Caller-Owned Sandbox](/docs/text2sql/sqlv3).
...
| Method                    | Description                                            |
| ------------------------- | ------------------------------------------------------ |
| `toSql(input, adapterName)` | Generate SQL without executing (routed to one adapter) |
| `index(options?)` | Introspect all configured adapters (cached per adapter); accepts `{ onProgress }` to stream events |
| `toPairs(adapterName, factory)` | Generate training pairs from one configured adapter |
...
- [Caller-Owned Sandbox](/docs/text2sql/sqlv3) - Compose the SQL sandbox extension with your own commands, plugins, and backends
