...
import { BriefCache, Sqlite, Text2Sql } from '@deepagents/text2sql';
...
| `single(input)`           | Stream SQL generation with execution                   |
| `chat(messages, params)`  | Multi-turn conversation with history and user profiles |
| `explain(sql)`            | Explain SQL in plain English                           |
| `teach(input)`            | Auto-generate teachables from context                  |
| `instruct(...teachables)` | Manually add teachables to the system                  |
| `suggest()`               | Get suggested questions for your database              |
| `inspect()`               | View the full system prompt with all context           |
