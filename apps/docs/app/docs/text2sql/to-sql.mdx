...
A `lock` is now required — pass `new FileIndexLock()`, or a distributed lock for
a fleet. See the package README's "Schema index caching & coordination" section
for the injected `cache`/`lock` model and its invalidation namespace.
