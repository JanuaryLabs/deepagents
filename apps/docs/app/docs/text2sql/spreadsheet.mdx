...
// Reuse model, filesystem, and context from Getting Started.
...
const sql = await text2sql.toSql('What are the top 5 products by revenue?');
...
console.log('SQL:', topCustomers);

const results = await adapter.execute(topCustomers);
