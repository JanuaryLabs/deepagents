...
```typescript
import { DepthEvolver } from '@deepagents/text2sql/synthesis';

const seedPairs = [
  { question: 'Show all orders', sql: 'SELECT * FROM orders', success: true }
];

const evolved = await new DepthEvolver(seedPairs, adapter, {
    count: 3,
...
const final = await new BreadthEvolver(complex, {
    count: 3,
  }).toPairs();
...
const level1 = await new DepthEvolver(seeds, adapter, {
    techniques: ['add-filter'],
  }).toPairs();

// Level 2: Add aggregations to filtered queries
const level2 = await new DepthEvolver(level1, adapter, {
    techniques: ['add-aggregation'],
  }).toPairs();

// Level 3: Add joins to aggregated queries
const level3 = await new DepthEvolver(level2, adapter, {
    techniques: ['add-join'],
  }).toPairs();

// Level 4: Add reasoning on top
const level4 = await new DepthEvolver(level3, adapter, {
    techniques: ['add-reasoning'],
  }).toPairs();
...
const depth = await new DepthEvolver([seed], adapter).toPairs();
...
const breadth = await new BreadthEvolver([seed], { count: 3 }).toPairs();
...
```typescript
import { DepthEvolver, BreadthEvolver } from '@deepagents/text2sql/synthesis';
...
// Evolve in depth
const complex = await new DepthEvolver(seeds, adapter, { count: 5 }).toPairs();

// Then evolve in breadth
const final = await new BreadthEvolver(complex, { count: 3 }).toPairs();
