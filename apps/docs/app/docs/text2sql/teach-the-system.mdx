...
Add fragments to the `ContextEngine` you pass to `Text2Sql`:
...
Those fragments are available to `chat()` because Text2SQL resolves the same
engine whenever it prepares a chat turn.
...
4. **Keep `chat()` and `toSql()` separate in your mental model**: `ContextEngine` fragments affect persistent chat flows, while direct SQL generation requires explicit fragments on each call.
