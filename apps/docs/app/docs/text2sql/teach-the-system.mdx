...
Text2SQL can use domain-specific fragments such as terms, hints, guardrails,
examples, and workflows. The current API does not expose mutable
`text2sql.instruct()` or `text2sql.teach()` helpers. Instead, you inject
...
All teachable fragments come from `@deepagents/context`.
...
4. **Keep chat() and toSql() separate in your mental model**: context factory fragments affect `chat()`, while direct SQL generation requires explicit fragments.
