...
```typescript
import { SchemaSynthesizer, toPairs } from '@deepagents/text2sql';
...
const low = await toPairs(new SchemaSynthesizer(adapter, {
  count: 20,
  complexity: 'low',
}));
...
const medium = await toPairs(new SchemaSynthesizer(adapter, {
  count: 20,
  complexity: 'medium',
}));
...
const hard = await toPairs(new SchemaSynthesizer(adapter, {
  count: 20,
  complexity: 'hard',
}));
...
const window = await toPairs(new SchemaSynthesizer(adapter, {
  count: 20,
  complexity: 'window',
}));
...
```typescript
import { SchemaSynthesizer, PersonaGenerator, toPairs } from '@deepagents/text2sql';
...
```typescript
import { TeachingsGenerator, SchemaSynthesizer, toPairs } from '@deepagents/text2sql';
...
const fastPairs = await toPairs(new SchemaSynthesizer(adapter, {
  count: 100,
  concurrency: 10,
}));

// Lower concurrency to reduce API rate limits
const slowPairs = await toPairs(new SchemaSynthesizer(adapter, {
  count: 100,
  concurrency: 2,
}));
```
...
```typescript
import { SchemaSynthesizer, BreadthEvolver, toPairs } from '@deepagents/text2sql';
...
const expanded = await toPairs(
  new BreadthEvolver(basePairs, { count: 3 })
);
