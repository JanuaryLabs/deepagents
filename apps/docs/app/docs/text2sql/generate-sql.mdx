...
Use `chat()` for interactive queries with streaming responses and conversation history:

```typescript
const stream = await text2sql.chat(
  [{ role: 'user', content: 'What genres have the most tracks?' }],
...
Plus memory tools (`remember_memory`, `recall_memory`, etc.) and any custom rendering tools you provide.
...
try {
  const stream = await text2sql.chat(
    [{ role: 'user', content: 'invalid query here' }],
    { chatId: 'chat-123', userId: 'user-456' },
  );
  // Process stream
} catch (error) {
