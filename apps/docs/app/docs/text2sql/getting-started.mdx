...
import { instructions, sqlSandboxExtension } from '@deepagents/text2sql';
...
2. **Builds SQL context** - `toSql()` uses cached schema fragments. The `chat()` helper from `@deepagents/context` wired to your text2sql index + instructions also resolves the stored conversation thread from your context store.
...
5. **Returns results** - `toSql()` returns the SQL string; the `chat()` helper from `@deepagents/context` wired to your text2sql index + instructions streams an end-user answer.

The `chat()` helper from `@deepagents/context` wired to your text2sql index +
instructions also injects Text2SQL's built-in SQL workflow guidance and any
extra fragments you add to your `ContextEngine`.
