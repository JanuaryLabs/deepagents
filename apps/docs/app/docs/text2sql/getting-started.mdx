...
1. **Loads grounding context** - Uses cached schema metadata (tables, columns, relationships, indexes, constraints)
2. **Applies teachables** - Uses domain knowledge (terms, hints, guardrails, examples)
3. **Applies step-back prompting** - Considers the broader question before generating SQL
4. **Validates the query** - Ensures the SQL is syntactically correct using `validate_query` tool
5. **Returns results** - Either raw SQL or executed results
