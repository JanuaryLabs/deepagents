...
`FileIndexCache`; without either, `sql index` introspects fresh every run. See
the package README's "Schema index caching & coordination" section for the
injected `cache`/`lock` model.
