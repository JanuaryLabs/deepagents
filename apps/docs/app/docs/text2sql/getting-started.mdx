...
Each example then creates `sandbox` by composing `createBashTool`, `createRoutingSandbox`, and `createVirtualSandbox({ fs: filesystem })` with `hostExtensions: [sqlSandboxExtension(adapter)]`.
