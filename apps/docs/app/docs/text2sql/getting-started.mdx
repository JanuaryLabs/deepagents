...
The `context` factory also fixes the conversation identity for `chat()`. Reuse
the same `store`, `chatId`, and `userId` when you want follow-up turns to stay
in the same thread. Create a new factory when you want a separate thread.
