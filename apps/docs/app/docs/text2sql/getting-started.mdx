...
PostHog uses native `fetch` and also requires no additional client package.

## Basic Setup

Text2SQL requires three components:

1. **Adapter** - Connects to your data source (PostgreSQL, SQLite, DuckDB, SQL Server, MySQL/MariaDB, BigQuery, ClickHouse, or PostHog)
2. **Grounding** - Configures database introspection (see [Grounding](/docs/text2sql/grounding) for advanced options)
3. **Model** - Any Vercel AI SDK language model
...
installed inside the sandbox. This is the configured connection name, not the
SQL-level database or schema name. When exactly one adapter is configured,
`sql run`/`sql validate` route to it regardless of the `<db>` token you pass, so
a single-database setup can use any name.
...
Adapter names must match `/^[A-Za-z_][A-Za-z0-9_]*$/`, and `database` is
reserved (it labels each fragment's connection name, below). If you construct
the adapter map dynamically, validate the keys first with
`isValidAdapterName(name)` or `validateAdapterNames(names)`.

When you index adapters for a chat agent, each schema fragment begins with a
`<database>` label carrying that adapter's configured connection name — the
exact value the model passes to `sql validate <db>` and `sql run <db>`. The
agent instructions point the model at that label, so it never has to infer the
name from the wrapper tag.

<Tabs groupId="database" items={['SQLite', 'PostgreSQL', 'DuckDB', 'SQL Server', 'MySQL/MariaDB', 'BigQuery', 'ClickHouse', 'PostHog']}>
<Tab value="SQLite">
```typescript
...
const adapter = new DuckDB({
  ...
  grounding: [duckdb.tables(), duckdb.info()],
});
...
const adapter = new ClickHouse({
  ...
  grounding: [clickhouse.tables(), clickhouse.info()],
});
...
const transport = createPostHogTransport({
  ...
  getAccessToken: () => getCurrentPostHogOAuthToken(),
});
const adapter = new PostHog({
  transport,
  grounding: [info(), schema(), definitions()],
});
...
Set `TEXT2SQL_INDEX_CACHE_DIR` (and optionally `TEXT2SQL_INDEX_VERSION` as an
invalidation namespace) to make the CLI back schema indexing with a
`FileIndexCache`; without either, `sql index` introspects fresh every run. The
CLI always wires a `FileIndexLock` to serialize concurrent indexing — its
directory comes from `TEXT2SQL_INDEX_LOCK_DIR`, falling back to
`TEXT2SQL_INDEX_CACHE_DIR`. See the package README's "Schema index caching &
coordination" section for the injected `cache`/`lock` model.
...
When the path is a FIFO, opening the writer blocks until a reader attaches.
...
The examples in this guide use Groq. If you follow them as written, set your
API key:
...
- [DuckDB Adapter](/docs/text2sql/duckdb) - Embedded analytics with native DuckDB policy analysis
- [BigQuery Adapter](/docs/text2sql/bigquery) - BigQuery-specific features
- [ClickHouse Adapter](/docs/text2sql/clickhouse) - ClickHouse-specific features
- [PostHog Adapter](/docs/text2sql/posthog) - PostHog HogQL setup and OAuth scopes
