...
description: Pre-built fragment bundle for clinical and healthcare databases with HIPAA compliance
---

This recipe provides a reusable fragment bundle for healthcare databases covering patient data, clinical metrics, and HIPAA-compliant analytics.
...
Spread `...healthcareRecipe` into your `context(...fragments)` factory for
`chat()`, or append it to the low-level `toSql({ fragments })` helper.
