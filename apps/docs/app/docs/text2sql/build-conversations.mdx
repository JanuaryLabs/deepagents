...
Messages accept `ChatMessage[]` where `ChatMessage` is `UIMessage | MessageFragment`. Standard `UIMessage` objects work as before:

```typescript
import type { ChatMessage } from '@deepagents/context';
...
Pass an `AbortSignal` to cancel an in-progress chat stream:
