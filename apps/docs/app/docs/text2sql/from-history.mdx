...
```typescript
import { MessageExtractor, toPairs } from '@deepagents/text2sql';
...
Or using the `Text2Sql` instance:
...
For multi-turn conversations where context matters, use contextual extractors:
...
// pairs[0].context = ['Show me sales data']
...
```typescript
import { WindowedContextExtractor } from '@deepagents/text2sql';

const extractor = new WindowedContextExtractor(messages, adapter, {
  windowSize: 3, // Keep last 3 messages as context
});
```

### SegmentedContextExtractor

Uses an LLM to detect topic changes and reset context:

```typescript
import { SegmentedContextExtractor } from '@deepagents/text2sql';
