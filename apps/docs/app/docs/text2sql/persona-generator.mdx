...
// 2. Define a manual fragment bundle
const teachings = [
  term('ARR', 'annual recurring revenue'),
  hint("Exclude sandbox accounts where account_type = 'test'"),
];

// 3. Generate base pairs with personas and the fragment bundle
...
3. **Combine with a fragment bundle** - Personas plus your stable domain fragments produce richer training data
