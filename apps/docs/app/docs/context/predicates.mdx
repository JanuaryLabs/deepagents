...
| `executingTool` | The live tool call being wrapped during `target: 'tool-output'` evaluation; undefined for user and steer reminders |
| `firedOnceIds` | Ids already latched by `once()` this conversation (steer + user); populated by the engine |
| `onceCollector` | Per-evaluation buffer `once()` appends to; the engine commits it only if the reminder fires |

`currentMessage` lets a predicate read metadata the user attached to *this* turn — e.g., `currentMessage.metadata.locale.timeZone` set on turn 1, before any prior `lastMessage` exists. `executingTool` is the only field that describes a tool call still in flight; `lastAssistantMessage` and `lastAssistantMessages` only describe already-persisted assistant history. The `firedOnceIds`/`onceCollector` fields back the `once()` latch and are managed by the engine — predicates compose `once(id)` rather than reading them directly.
...
These predicates inspect tool parts on `lastAssistantMessage`, so they describe completed tool calls that are already in the conversation history. For `target: 'tool-output'` reminders that need to gate on the live tool call being wrapped, read `ctx.executingTool` directly in a custom predicate.
