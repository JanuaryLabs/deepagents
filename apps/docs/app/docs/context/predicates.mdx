...
These predicates inspect tool parts on `lastAssistantMessage`, so they describe calls visible in the current assistant segment. During steer evaluation, the engine also includes terminal tool parts from the just-completed step. Use `toolOutput()` instead when a tool-output reminder needs the raw outcome currently being evaluated.
