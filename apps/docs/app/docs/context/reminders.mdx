...
| `reminder(fragment: ContextFragment)` | `true` (part) |

String reminders fall inline so the model sees them next to the user's words. Fragment reminders default to part mode because the fragment is pre-rendered to XML — keeping that block as its own part avoids polluting the user's text with structured markup. Override either default with `{ asPart }`:

```ts
reminder('hint');                    // inline (string default)
reminder('hint', { asPart: true });  // forced part

reminder(workflow({ task: 'Deploy', steps: ['…'] }));                  // part (fragment default)
reminder(workflow({ task: 'Deploy', steps: ['…'] }), { asPart: false }); // forced inline
```
