...
| `tool-output` | Right after a tool `execute()` resolves | Wrapped tool output on the next model step; the persisted chain also keeps a host-only metadata marker so reminder envelopes can be unwrapped safely |
...
During `tool-output` evaluation, the predicate context includes `ctx.executingTool` with the live tool name, input, and raw output being wrapped. Use it when a reminder should fire only for a specific tool call; `lastAssistantMessage` and `lastAssistantMessages` only describe already-persisted assistant history.
...
engine.set(
  reminder('Double-check filesystem side effects', {
    when: (ctx) =>
      ctx.executingTool?.name === 'bash' &&
      typeof ctx.executingTool.input === 'object' &&
      ctx.executingTool.input !== null &&
      'command' in ctx.executingTool.input &&
      String(ctx.executingTool.input.command).startsWith('rm '),
    target: 'tool-output',
  }),
...
interface ReminderContext {
  ...
  lastAssistantMessages?: UIMessage[]; // Recent persisted assistant messages
  executingTool?: {           // Only for target: 'tool-output'
    name: string;
    input: unknown;
    output: unknown;
  };
