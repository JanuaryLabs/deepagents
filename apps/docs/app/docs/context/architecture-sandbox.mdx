...
To give an AI agent a bash tool that runs inside Docker, compose the two
...
The `DockerSandbox` interface still matches what `createBashTool` expects from
its `sandbox` parameter (`executeCommand(command, options?) → { stdout, stderr,
exitCode }`), while also exposing Docker-only `spawn(command, options?)` for
