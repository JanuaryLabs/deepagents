...
Docker, Apple Container, Agent OS, and Daytona, also implement optional
`spawn(command, options?)` for streaming stdout/stderr. This is why real process
backends can be plugged directly into the shared `createBashTool` from
`@deepagents/context`, which in turn delegates to the upstream `bash-tool`
package.
