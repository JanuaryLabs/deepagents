...
- **Writes are eventually consistent.** With `vfs-cache-mode=writes`, writes land in a local cache and upload asynchronously after the file closes and the `--vfs-write-back` delay elapses (rclone default `5s`; the volume above uses `1s`), with a final flush on unmount. The upload is async even at `0s`, so a reader on the GCS side must poll for the object or read through the mount. A low `vfs-write-back` costs extra uploads when the same file is re-written rapidly (no coalescing) — fine for whole-file create-and-close writes.
