...
For a deep-dive into internals, see [Architecture](/docs/context/architecture-core-concepts).
...
```typescript
import { assistantText, hint, role, user } from '@deepagents/context';
...
// Chainable
context.set(role('You are helpful.')).set(user('Hello!'));
```
...
#### createPrepareStep(options?)

Build the AI SDK v7 `prepareStep` hook used by Context agents. The hook evaluates
terminal tool outcomes before the next model generation and, by default,
evaluates steer reminders after the first completed model step.
...
Pass `{ steer: false }` to disable steer evaluation while retaining tool-output
reminders. Returned message overrides carry forward to later AI SDK steps.

`agent()` wires this hook automatically. Direct AI SDK users are responsible
for persisting generated assistant messages; use `writeAssistantSegment()` when
the stored chain must reproduce the reminder boundaries sent to the model.

#### writeAssistantSegment(message, options?)

Persist a cumulative streamed assistant `UIMessage` into the assistant head
reserved by `continue()`. When `createPrepareStep()` fired model-only reminders,
this method stores the corresponding
`assistant → synthetic reminder → assistant` segments. `chat()` calls it
automatically; it is primarily exposed for custom stream integrations.

In `onEnd`, abort, and failure cleanup, pass `{ final: true }`. This persists a
reminder that was already sent to the model even when the provider terminates
before emitting the following step marker. A failure handler should pass the
latest cumulative assistant message it retained from the stream.

```typescript
await context.writeAssistantSegment(responseMessage, { final: true });
```
...
Create an isolated child `ContextEngine` with the same non-message fragments and
resolver chain, but a fresh in-memory store and no conversation history. The child
gets a random `chatId`, so the run disappears with the process.

```typescript
import { XmlRenderer, user } from '@deepagents/context';

const child = context.fork();
child.set(user('Draft three headline options for this article.'));

const { systemPrompt, messages } = await child.resolve({
  renderer: new XmlRenderer(),
});
```

Use `fork()` for one-shot helper flows such as `agent.asTool()` where you want
to preserve the parent system context without mutating the parent chat thread.

A forked child is deliberately not addressable. If you need a durable child run —
one you can list, resume, or attribute back to its parent — create a chat of its
own rather than forking.
