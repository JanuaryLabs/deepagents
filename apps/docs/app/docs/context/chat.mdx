...
The `generateTitle` option is part of `ChatOptions`, which also accepts `transform`, `abortSignal`, `onError`, `messageMetadata`, and `finalAssistantMetadata`. `toolsContext` is required only when an agent tool declares context.
...
for (const chat of chats) {
  ...
  console.log(
    `  Messages: ${chat.messageCount}, Branches: ${chat.branchCount}`,
  );
...
Each call to `trackUsage()` adds to the running total. Concurrent calls are safe: the store runs the read, accumulation, and write as one transactional `updateChat` operation while holding the chat row's write lock.
...
- [chat()](/docs/context/chat-function) - Streaming orchestration with automatic persistence, titles, and usage tracking
