...
Four layers handle this:

- **StreamStore** — abstract storage interface for streams and chunks
- **StreamChangeSource** — how `StreamManager` learns that chunks/status changed
...
| `data` | `StreamPart` | The AI SDK stream part (serialized as JSON in SQLite) |
...
const store = new SqliteStreamStore('./streams.db');
const changeSource = new PollingChangeSource({ reads: store });
const manager = new StreamManager({ store, changeSource });
...
Use `PollingChangeSource` for SQLite/local polling. For Postgres deployments
that use LISTEN/NOTIFY, swap in `PostgresNotifyChangeSource`:
...
const store = new PostgresStreamStore({ pool: process.env.DATABASE_URL! });
...
Consumes a `ReadableStream`, persists every chunk to the store, and transitions status from `running` to a terminal state. Streams fail in two ways:

- `persist()` throws while draining the source stream
- Any persisted chunk has `type: 'error'` (the store marks the stream as `failed` and records `errorText`)
...
const uiStream = createUIMessageStream({
  execute: async ({ writer }) => {
    ...
    writer.write({ type: 'text-delta', id: 'text-1', delta: ' world' });
  },
});
...
await manager.persist(uiStream, 'stream-abc', {
  strategy: 'buffered',
  flushSize: 20,
  onCancelDetected: ({ streamId, latencyMs }) => {
    console.log('cancel detected', streamId, latencyMs);
  },
});
```
...
| `onCancelDetected` | `({ streamId, latencyMs }) => void \| Promise<void>` | `undefined` | Optional callback when cancellation is detected |
...
Polling/backoff behavior is configured on the `StreamChangeSource` (for polling, via `PollingChangeSource.config`). `chunkPageSize` is configured on `new StreamManager({ chunkPageSize })`.

Throws if the stream ID does not exist.

If a stream is deleted while being watched, the watcher closes gracefully.

### StreamManager telemetry

Use `onWatchEvent` to observe `watch()` and cancellation behavior:
...
const manager = new StreamManager({
  ...
  changeSource,
  onWatchEvent: (event) => {
    if (event.type === 'watch:chunks') {
      console.debug('delivered', event.delivered);
    }
  },
});
```
...
### PollingChangeSource telemetry

Use `PollingChangeSource.onPoll` to tune poll cadence:

```typescript
import { PollingChangeSource, SqliteStreamStore } from '@deepagents/context';
...
const uiStream = createUIMessageStream({
  execute: async ({ writer }) => {
    ...
    pw.writer.write({ type: 'text-delta', id: 'text-1', delta: 'Hello' });
    pw.writer.write({ type: 'text-delta', id: 'text-1', delta: ' world' });
...
| `complete()` | Flush remaining chunks and mark stream as `completed` (unless an `error` chunk already marked it `failed`) |
| `fail(error?)` | Flush remaining chunks and mark stream as `failed` with optional error message (unless an `error` chunk already set failure state) |
...
- Treat `type: 'error'` chunks as terminal failure signals. Emit them only for unrecoverable stream failures.
- Always call `cleanup()` after you no longer need a stream. Chunks accumulate and are not automatically garbage-collected.
- Tune `PollingChangeSource.config` and `StreamManager.chunkPageSize` for your latency and DB-load budget. Lower intervals reduce latency but increase polling cost.
