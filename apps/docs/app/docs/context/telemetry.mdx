...
Parent directories are created automatically, and existing files are preserved.
Writes from the same integration instance are serialized, so concurrent AI
events cannot interleave and corrupt JSONL records.
...
Both integrations emit the same record shape with the ISO timestamp captured
...
`runtimeContext` is redacted with other inputs.
