...
Per-call integrations replace the globally registered integrations for that
call. When overriding them, include every destination the call should receive:
...
| `includeTimestamp`       | `boolean`                                       | `true`   | Add an ISO timestamp to each record                                                       |
| `preserveRuntimeContext` | `readonly string[]`                             | `[]`     | Preserve named runtime-context namespaces when inputs are not recorded                    |
...
The returned integration exposes `telemetry.traces.path` as an absolute `file:`
URL. It writes to that same resolved file, allowing runtimes to advertise the
trace source without duplicating the configured path.
...
`runtimeContext` is normally redacted with other inputs. Use
`preserveRuntimeContext` only for namespaces containing non-sensitive
correlation identifiers. For example, the Zukhruf devtool uses
`preserveRuntimeContext: ['zukhruf']`; every other runtime-context namespace
remains redacted.
