...
description: Build just-bash command groups with dispatch, quote repair, and structured errors
---
...
Subcommand handlers compose cleanly with other sandbox APIs:

- `BashException` lets a handler fail with a structured `CommandResult` instead of throwing a generic error.
- `onAfterBashCall` can attach host-only `meta` to a successful command result.
