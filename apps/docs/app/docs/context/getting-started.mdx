...
The package uses the built-in `node:sqlite` module in Node.js LTS for persistence, so no additional database drivers are required.
...
- `assistantText()` - Assistant text responses (auto-persisted)
- `assistant()` - Wrap an existing assistant `UIMessage` (auto-persisted)
...
// [{ id: '...', role: 'user', parts: [{ type: 'text', text: 'How do I read a file in Node.js?' }] }]
```
...
async function main() {
  ...
  context.set(assistantText(response.text));
...
Messages marked with `persist: true` are saved when you call `save()`. The `user()`, `assistantText()`, and `assistant()` helpers automatically set this flag:
...
// ... get AI response
context.set(assistantText('Hi there! How can I help?'));
...
//   { id: '...', role: 'assistant', parts: [{ type: 'text', text: 'Hi there! How can I help?' }] }
// ]
```
...
The estimate uses real pricing information from [models.dev](https://models.dev) for 1000+ models.
