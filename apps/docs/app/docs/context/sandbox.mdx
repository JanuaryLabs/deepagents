---
title: Sandbox
description: Execute real sandbox commands across Docker, Daytona, WASM, virtual, and custom backends
---

The sandbox system lets AI agents execute real commands in isolated
environments. Choose between Docker containers for full binary execution,
...
(`createDockerSandbox` / `createDaytonaSandbox` / `createAgentOsSandbox` /
`createVirtualSandbox`), then wrap it with `createBashTool({ sandbox })` to
get the bash tool, file tools, and skills upload. For per-command file-change
tracking, first decorate a real backend with `withStraceFileChanges(...)`, then
pass that tracked backend into `createBashTool`.
