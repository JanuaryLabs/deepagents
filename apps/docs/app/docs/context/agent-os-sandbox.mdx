...
The Agent OS sandbox runs commands in an in-process WASM virtual machine. No
Docker daemon required. Cold start is ~6ms. It implements the same core
buffered sandbox surface as Docker (`executeCommand`, `readFile`, `writeFiles`,
`dispose`), so most command/file flows can switch backends without refactoring.
It also exposes `sandbox.spawn(...)` for live stdout/stderr streams from Agent
OS processes.
...
Since `AgentOsSandbox` implements the `DisposableSandbox` interface from `@deepagents/context`, the same owned tools work with both Docker and WASM backends without changes.
...
### Scope-Based Disposal with `await using`

Every backend implements `[Symbol.asyncDispose]`, so an `await using` declaration
disposes the sandbox automatically when the block exits — no `try/finally`:

```typescript
await using sandbox = await createAgentOsSandbox({ software: [common] });
await sandbox.executeCommand('ls /');
// disposed at scope exit, even if the block throws
```
