...
```typescript
import { chat, agent, ContextEngine, SqliteContextStore, role } from '@deepagents/context';
...
interface ChatOptions<CIn> {
  ...
  >['messageMetadata'];
  finalAssistantMetadata?: (message: UIMessage) =>
...
```typescript
import { toMessageFragment, chatMessageToUIMessage } from '@deepagents/context';
