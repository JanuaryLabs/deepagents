...
Apple's [`container`](https://github.com/apple/container) CLI. Unlike the Docker
...
**Default image:** `docker.io/library/bash:5.3-alpine3.24`. Prefer fully-qualified refs — `container` resolves short names against configured registries.
...
```typescript
await using sandbox = await createAppleContainerSandbox({
  image: 'docker.io/library/node:lts-bookworm-slim',
