...
Five atoms cover time-derived facts. Each accepts `{ tz?: string }`, uses `asPart: true` (adds a separate text part), and computes diffs purely from `lastMessageAt` — no metadata storage. When `tz` is omitted, the atom resolves the timezone from message metadata; see [Timezone resolution](#timezone-resolution).
