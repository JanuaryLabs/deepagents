...
context.set(assistantText('Great! Would you prefer Python or JavaScript?'));
await context.save();
...
context.set(assistantText('Python is great for beginners!'));
await context.save();
...
context.set(assistantText('JavaScript is excellent for web development!'));
await context.save();
...
context.set(assistantText('What features do you need?'));
await context.save();
await context.checkpoint('requirements');

// Stage 2: Design
context.set(user('I need tasks with due dates and priorities'));
context.set(assistantText('Here is the proposed architecture...'));
await context.save();
await context.checkpoint('design');

// Stage 3: Implementation
context.set(user('Start with the domain model'));
context.set(assistantText('Here is the schema...'));
await context.save();
...
Checkpoints appear in the graph snapshot:
