...
`Telemetry` integration for the AI SDK. Pass it to `telemetry`
...
Returns an AI SDK `Telemetry` implementation compatible with the
stable `telemetry` integration option. It listens for `onStart`,
`onStepStart`, `onStepEnd`, `onToolExecutionStart`, `onToolExecutionEnd`, and
`onEnd` events and emits one trace per run with nested spans.
...
| `onStepEnd` | closes the generation span | Usage tokens, response messages |
| `onToolExecutionStart` | `function` | Tool name + JSON-stringified input |
| `onToolExecutionEnd` | closes the function span | JSON-stringified output, or error |
| `onEnd` | closes the root span | Total usage, step count, finish reason |

When multiple runs are open concurrently (nested agents, `asAdvisor`),
events are routed to the correct run by the AI SDK v7 `callId`, then by
`response.id` or `toolCall.toolCallId` when available. When exactly one run is
open, it gets an otherwise uncorrelated event directly. When no run is open,
the event is dropped rather than attached to the wrong trace.
...
- [Agent](/docs/context/agent) — `telemetry` on the agent options
