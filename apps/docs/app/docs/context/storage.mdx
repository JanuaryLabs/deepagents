...
File-based persistence using the native SQLite module in Node.js LTS.
...
**Requirements:**
- Node.js LTS (uses `node:sqlite` module)
...
<pre><code className="language-sql">{`IF OBJECT_ID('[dbo].[chats]', 'U') IS NULL
BEGIN
...
<pre><code className="language-prisma">{`datasource db {
