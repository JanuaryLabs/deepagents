...
It also ships the portable sandbox layer used by the rest of the repo: routing
host-dispatched commands across virtual, Docker, and Agent OS backends, plus
helpers for composing validated command groups from OpenAPI specs.
...
| Sandbox Tooling             | `createRoutingSandbox()`, `SandboxExtension`, and `createOpenAPIExtension()` for portable host commands |
...
- [Routing Sandbox](/docs/context/routing-sandbox) - Run the same host commands across multiple backends
