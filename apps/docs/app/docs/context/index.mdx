---
title: Overview
description: Domain-agnostic context management for LLM applications with rendering, persistence, chat orchestration, and sandbox tooling
---

The `@deepagents/context` package provides primitives for composing LLM
context: organizing fragments, rendering them to multiple prompt formats,
persisting conversations, orchestrating chat flows, and estimating usage
costs. It is the foundation layer that other DeepAgents packages build upon.

It also ships the sandbox layer used by the rest of the repo: direct virtual
execution, Docker containers, Apple Container lightweight VMs, Agent OS
backends, Microsandbox microVMs, per-command file-change tracking, and the
shared bash tool wrapper for agents.
...
// messages: [{ id: '...', role: 'user', parts: [{ type: 'text', text: 'What is TypeScript?' }] }]
```
...
| Fragment Serialization | `fromFragment()`, `toFragment()`, `encodeSerializedValue()` for storing non-message fragments as JSON |
...
| Sandbox Tooling             | `createBashTool()`, `createVirtualSandbox()`, `createDockerSandbox()`, `createAppleContainerSandbox()`, `createMicrosandboxSandbox()`, and real command execution backends |
...
const domainContext = new ContextEngine({ store }).set(
  fragment(
    'domain',
...
- [Fragment Serialization](/docs/context/fragment-serialization) - Store non-message fragments in your own database
- [Sandbox](/docs/context/sandbox) - Execute commands through virtual, Docker, Apple Container, Microsandbox, or Agent OS backends
- [Microsandbox Sandbox](/docs/context/microsandbox-sandbox) - Run OCI images in local hardware-isolated microVMs
