...
Every agent requires a **sandbox** — an `AgentSandbox` that provides `bash`, `readFile`, and `writeFile` tools. The sandbox is created externally and passed in; the agent does not create or dispose it. Skills are declared separately on the context with `skills(availableSkills)`.

## Sandbox Options

| Approach                                                     | When to use                                 | Docker? |
| ------------------------------------------------------------ | ------------------------------------------- | ------- |
| `createVirtualSandbox()` + `createBashTool()`                | Local dev, tests, lightweight tasks         | No      |
| `createDockerSandbox()` + `createBashTool()`                 | Real binaries in isolated Docker containers | Yes     |
| `createDaytonaSandbox(client, options)` + `createBashTool()` | Managed Daytona sandboxes                   | No      |
| `createBashTool({ sandbox: e2bSandbox })`                    | Other cloud sandboxes via adapter           | No      |

`createBashTool` is owned by `@deepagents/context`; it adds AI SDK bash and file tools to an explicit backend and returns an `AgentSandbox`. It has no skills-specific behavior.
...
```typescript
import { createBashTool, createDockerSandbox, pkg } from '@deepagents/context';

// Pick the backend, then layer the bash tool.
...
```typescript
import { bin, createBashTool, createDockerSandbox } from '@deepagents/context';
...
The command name defaults to the binary basename without its extension (`sql.js` → `sql`). Override with `{ name }` or `{ target }`:
...
// Pass a caller-owned Daytona client plus either a stable `name`
// (get-or-create) or `sandboxId` (attach). Authentication, base URL, and
// other Daytona config live on `new Daytona(...)`, not on
// `createDaytonaSandbox(...)`. The sandbox lifecycle stays with that client.
const backend = await createDaytonaSandbox(client, {
  name: 'agent-backend',
  image: 'your-registry/agent-image:latest',
});
...
`dispose()` detaches local handles but does not delete a Daytona sandbox. Reuse
stable `name` values when you want get-or-create semantics across runs, or pass
`sandboxId` to attach to a specific sandbox directly. An unnamed sandbox cannot
be reclaimed, so `createDaytonaSandbox(...)` requires one of those two
identifiers.

The agent merges `sandbox.tools` into its toolset automatically. Any `tools` you pass override sandbox tools on name conflict.

## Host-only Tool Metadata

Any object-returning tool can include a top-level `meta` field for information
the application should retain without sending it back to the model. No
per-tool `toModelOutput` is required; the agent applies the projection to its
entire toolset for generation, streaming, and persisted-history replay.
...
Host-side tool results and UI messages retain both `status` and `meta`. The
model sees only `{ status: 'ready' }`. A tool that defines its own
`toModelOutput` keeps that custom projection.

Host-only means hidden from the model, not secret: metadata can still be
streamed, persisted, logged, or sent to a client. Keep sensitive values in a
server-side store and put only an opaque reference in `meta`.
...
| `sandbox`             | `AgentSandbox`    | required           | Sandbox providing bash, readFile, and writeFile tools — created via `createBashTool()` (optionally chained on top of `createDockerSandbox()` or `createDaytonaSandbox(client, options)`) from `@deepagents/context` |
| `context`             | `ContextEngine`   | `undefined`        | Engine that resolves system prompt and messages                                                                                                                                                                     |
| `model`               | `LanguageModelV4` | `undefined`        | AI SDK model to use for generation                                                                                                                                                                                  |
...
| `telemetry`           | `object`          | `undefined`        | AI SDK telemetry settings forwarded unchanged to `generateText` and `streamText`                                                                                                                                    |
| `stopWhen`            | `StopCondition`   | `isStepCount(200)` | Ends the agent loop. Raise it for a long agentic run that would otherwise stop mid-task; lower it to bound cost on an untrusted prompt                                                                              |
...
| `model`   | `LanguageModelV4 \| undefined` | The model configured for this agent         |
...
`generate()` accepts one options object:

| Argument             | Type                  | Description                                      |
| -------------------- | --------------------- | ------------------------------------------------ |
| `toolsContext` | `InferToolSetContext` | Required only when a tool declares context; keyed by tool name |
| `abortSignal`  | `AbortSignal`         | Cancel the request                                          |

`abortSignal` is forwarded to the model call and any tool-call repair attempt, so cancellation stops the full request instead of only the outer wrapper.
...
`stream()` accepts one options object:

| Argument             | Type                                           | Description                                             |
| -------------------- | ---------------------------------------------- | ------------------------------------------------------- |
| `toolsContext` | `InferToolSetContext`                          | Required only when a tool declares context              |
| `abortSignal`  | `AbortSignal`                                  | Cancel the request                                      |
| `transform`    | `StreamTextTransform \| StreamTextTransform[]` | Custom stream transforms (defaults to `smoothStream()`) |
| `maxRetries`   | `number`                                       | Override `maxGuardrailRetries` for this call            |

`abortSignal` is reused across repair attempts and guardrail retries, so cancelling a stream stops the active turn cleanly.
