...
| `createDaytonaTool()` | Managed Daytona cloud sandboxes | No | Yes |
...
```typescript
import { bin, createContainerTool } from '@deepagents/context';
...
```typescript
import { createDaytonaTool } from '@deepagents/context';

// Daytona defaults to /home/daytona and reads DAYTONA_* env vars
const sandbox = await createDaytonaTool();
// Don't forget: await sandbox.sandbox.dispose() when done
```
