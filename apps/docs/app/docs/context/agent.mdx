...
| ------------------------------------------------------------ | ------------------------------------------- | ------- | ----------------- |
| `createVirtualSandbox()` + `createBashTool()`                | Local dev, tests, lightweight tasks         | No      | Yes               |
| `createDockerSandbox()` + `createBashTool()`                 | Real binaries in isolated Docker containers | Yes     | Yes               |
| `createDaytonaSandbox(client, options)` + `createBashTool()` | Managed Daytona sandboxes                   | No      | Yes               |
| `createBashTool({ sandbox: e2bSandbox })`                    | Other cloud sandboxes via adapter           | No      | Yes               |

`createBashTool` is owned by `@deepagents/context`; it adds the AI SDK tools to an explicit backend, supports skill uploads, and returns an `AgentSandbox`.
...
| `sandbox`             | `AgentSandbox`    | required    | Sandbox providing bash, readFile, writeFile tools plus `skills` — created via `createBashTool()` (optionally chained on top of `createDockerSandbox()` or `createDaytonaSandbox(client, options)`) from `@deepagents/context` |
