...
// Pass an existing Daytona client plus either a stable `name` (get-or-create)
// or `sandboxId` (attach). The sandbox lifecycle stays with that client.
...
`sandboxId` to attach to a specific sandbox directly.
