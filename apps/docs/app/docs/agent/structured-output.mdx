...
const { output } = await generate(agent, 'Analyze sentiment', {});
// output is typed as:
...
const { output } = await generate(analyst, data, {});

console.log(output.summary);
console.log(output.findings); // string[]
console.log(output.recommendations); // string[]
```
...
// output is typed as Plan
const { output } = await generate(planner, task, {});
```
