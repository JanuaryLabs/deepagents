...
Invalid tool calls happen when models generate malformed arguments. DeepAgents automatically repairs them using the agent's own model. Each agent creates its repair function at construction time from the model you pass in:

```typescript
import { createRepairToolCall } from '@deepagents/agent';

const repair = createRepairToolCall(model);
// repair is called automatically when tool validation fails
```

The repair function works as follows:

1. If the tool name does not exist (`NoSuchToolError`), repair is skipped
2. Otherwise, the agent's model is called with the invalid input and the tool's schema
3. The model produces corrected input that matches the schema
4. Execution continues with the repaired tool call

Because each agent uses its own model for repair, agents with stronger models get more reliable tool call recovery. You do not need to configure this -- it is handled automatically based on the model you pass to `agent()`.
...
- [Handoffs](/docs/agent/orchestration/handoffs) - How agents delegate to each other
