...
const riskAnalyst = agent({
  ...
  prompt: instructions({
    ...
    routine: [
      'Identify competitive threats, regulatory issues, supply chain problems',
    ],
  }),
  handoffDescription:
    'Use to get a short write-up of potential red flags in financial data',
});
```
...
const financialAnalyst = agent({
  ...
  prompt: instructions({
    purpose: [
      'You perform financial analysis by gathering data from specialists.',
    ],
    routine: [
...
const writer = agent({
  ...
  output: z.object({
    ...
    follow_up_questions: z
      .array(z.string())
      .describe('Suggested follow-up topics'),
  }),
});
