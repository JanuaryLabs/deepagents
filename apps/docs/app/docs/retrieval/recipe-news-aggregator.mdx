...
const DigestSchema = z.object({
  ...
  categories: z.record(z.array(z.string())).describe('Stories grouped by category'),
});
...
async function generateDigest() {
  ...
  const topics = ['AI', 'startups', 'cybersecurity', 'developer tools', 'tech industry'];
  ...
  const unique = [...new Map(allResults.map(r => [r.document_id, r])).values()];
  const top = unique
...
---`).join('\n');
  ...
  const { experimental_output: digest } = await generate(
    digestAgent,
...
async function main() {
  ...
  for (const result of searchResults) {
    console.log(`[${result.similarity.toFixed(2)}] ${result.content.slice(0, 200)}...\n`);
  }
}
