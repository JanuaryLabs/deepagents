...
2. **Explore** — Inspect the real entry points and call sites. Spawn a
   `coding-team:code-explorer` only for a focused, independent question, then
   wait for it.
...
4. **Design** — Reuse the closest existing module. Spawn
   `coding-team:code-architect` only when the change has a genuine
   architectural choice, then wait for it.
...
7. **Review** — Spawn `coding-team:code-reviewer` against the completed diff,
   wait for it, and repair confirmed high-impact findings before summarizing.
