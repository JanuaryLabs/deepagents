...
- **ALWAYS** ask to split when staged changes span unrelated concerns
- **ALWAYS** write subject lines that are business-friendly (user-facing impact) when there is a user-facing change; use `Technical:` section in body for implementation details
...
- **NEVER** add `Co-Authored-By` trailers
- Use `--no-verify` when pre-commit hooks fail due to staging conflicts (e.g., AD file status causing lint-staged merge errors), not to skip linting
