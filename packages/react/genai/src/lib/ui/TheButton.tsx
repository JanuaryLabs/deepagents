...
function IconRenderer({ icon }: { icon: React.ReactNode | LucideIcon }) {
  ...
  // Not a useless fragment: `isComponent` takes `unknown` and narrows only its
  // positive branch, so `icon` is still `ReactNode | LucideIcon` here and
  // `LucideIcon` is not assignable to `ReactNode`. The fragment is what makes
  // this an Element. Returning `icon` bare fails with TS2786 at both call sites.
  // eslint-disable-next-line react/jsx-no-useless-fragment -- carries the type, see above
  return <>{icon}</>;
}
