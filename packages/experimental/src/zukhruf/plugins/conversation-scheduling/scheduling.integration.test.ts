...
test('one conversation-scheduling definition creates a fresh instance per runtime', async (t) => {
  const scheduler = new RecordingWakeScheduler();
  const h = harness(t, scheduler);
  const plugin = conversationScheduling();
